 ...
 */
 export default function StartScreen(props) {
  ...
  function setMyName() {
      ...
      if (usersWithAccess.includes(myState.name)) {
        ...
        begin();
      } else if(myState.name==='Username' || myState.name==='') {
        alert('Please enter your username.');
      } else {
        alert(`Sorry, username "${myState.name}" does not have access to this meeting.`)
      }
  }
useEffect(()=>{
                 ...
                 { hasUserMedia && (
                    <h4 className='signin'>
                        ...
                        <form onSubmit={(e)=>e.preventDefault()}>
                            <input type="text" autoComplete="off" style={{backgroundColor: 'rgb(230, 240, 249)', fontSize:'15px', width: '200px', height: '30px', textAlign:'center', fontFamily:'poppins'}} ref={userIDEl} value={myState.name} onFocus={()=> {if (myState.name==='Username') {setName('')}}} onChange={e=>handleChange(e)} /><br/><br/>
